...
| `skills.rs` | The declarative skill system: a folder of `<name>/SKILL.md` (frontmatter `name`/`description` + body). Renders the catalog shown each turn; the `skill_list` / `skill_apply` / `skill_file` tools; an LRU read-through cache so only applied bodies (not the whole folder) sit in context. |

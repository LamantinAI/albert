...
| `commands.rs` | The declarative `[commands]` reflex: a config-mapped `/name` runs a skill script via forkd and renders its JSON stdout through a `{field}` template — no LLM. Enforces `owner_only` (reusing `acl::is_owner`) upstream of dispatch; `help_listing` feeds the cogitator's `/help`. |
